...
YOLODetector::YOLODetector(const std::string& model_path, Camp camp, float conf_threshold, float nms_threshold, const std::string& device)
...
// 执行推理和后处理
std::vector<YoloArmor> YOLODetector::operator()(const cv::Mat& raw_img) {
...
// 可视化函数
void YOLODetector::draw(cv::Mat& img, const std::vector<YoloArmor>& detections) const {
...
// Letterbox 图像预处理
cv::Mat YOLODetector::letterbox(const cv::Mat& source, float& scale, int& pad_w, int& pad_h) const {
